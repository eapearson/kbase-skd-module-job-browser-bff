    ...
    def set_sheet(self, sheet):
        if isinstance(sheet, int):
            if sheet >= len(self._xl.sheet_names()) or sheet < 0:
                raise IndexError("Workbook has no sheet for index {}".format(sheet))
...
def convert_xl_row(xlrdbook, xlrdsheetname, rownum):
    ...
    for i, cell in enumerate(xlrdsheet.row(rownum)):
        if cell.ctype == _xlrd.XL_CELL_EMPTY or cell.ctype == _xlrd.XL_CELL_BLANK:
            ret.append(None)
        elif cell.ctype == _xlrd.XL_CELL_NUMBER or cell.ctype == _xlrd.XL_CELL_TEXT:
