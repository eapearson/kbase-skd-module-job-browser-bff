...
def raw_job_to_state(raw_job):
    ...
    if raw_state == "created":
        return {"status": "create", "create_at": raw_job["created"]}
    if raw_state == "queued":
        return {
            "status": "queue",
        ...
        state = {
            "status": "error",
            "create_at": raw_job["created"],
            "error": {"code": 1, "message": raw_job.get("errormsg", "")},
            "client_group": client_group,
        }
        ...
        if "finished" in raw_job:
            ...
            raise ValueError('"finished" timestamp required for "terminated" job')
    ...
    if "job_input" in raw_job:
        ...
        if job_type == "narrative":
            job = {
                ...
                "owner": {"username": raw_job["user"], "realname": realname},
                "context": {
                    "type": "narrative",
                    "workspace": {
                        "id": int(workspace_id),
            ...
            job = {
                ...
                "owner": {"username": raw_job["user"], "realname": realname},
                "app": app,
                "context": {"type": "workspace", "workspace": ws},
            }
        else:
            job = {
                "job_id": raw_job["job_id"],
...
class EE2Model(object):
    ...
    def cancel_job(self, params):
        ...
        try:
            ...
            if re.search(
                (
                    "A job with status .+ cannot be terminated. "
    ...
    def ee2_query_jobs(
        ...
        if filter is not None:
            ...
            if len(client_group) > 0:
                raw_query.append(
                    {"job_input.requirements.clientgroup": {"$in": client_group}}
                )
        ...
        if sort is not None:
            ...
            if len(sort) == 0:
                ...
                raise ServiceError(
                    code=40000, message="Only one sort spec supported", data={}
                )
        ...
        try:
            ...
            raise ServiceError(
                code=40000, message="Unknown error", data={"original_message": str(ex)}
            )
    ...
    def ee2_get_jobs(self, params):
        ...
        try:
            jobs = api.check_jobs({"job_ids": params["job_ids"], "as_admin": as_admin})
            ...
            if se.code == -32000:
                if re.search("Cannot find job with ids:", se.message):
                    ...
                    raise ServiceError(
                        code=10, message="Job not found", data={"message": se.message}
                    )
                else:
                    raise ServiceError(
                        code=1,
            ...
            raise ServiceError(
                code=1, message="Unknown error", data={"original_message": str(ex)}
            )
