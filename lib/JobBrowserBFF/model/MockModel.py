...
class MockModel(object):
    ...
    def is_admin(self):
        if self.token == "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA":
            ...
            raise Exception(
                "Unexpected mock condition: token not supported: {}".format(self.token)
            )
        ...
        for job in jobs:
            ...
            if not job["context"].get("workspace"):
                ...
                if workspace["is_accessible"]:
                    if workspace.get("narrative"):
                        job_type = "narrative"
                    elif job.get("app") and "export" in job["app"].get(
                        "function_name", ""
                    ):
                        # elif app is not None and 'export' in app['function_name']:
                        job_type = "export"
            ...
            if job_type == "narrative":
                context = {
                    ...
                    "narrative": {"title": workspace["narrative"].get("title", None)},
                }
    ...
    def query_jobs(self, params):
        ...
        if is_admin:
            if not self.is_admin():
                raise ServiceError(
                    code=50, message="Permission denied for this operation", data={}
                )
        else:
    ...
    def get_users(self, user_ids):
        ...
        if response.status_code != 200:
            raise ServiceError(
                code=40000, message="Error fetching users", data={"user_id": user_ids}
            )
        else:
            try:
                ...
                raise ServiceError(
                    ...
                    data={"user_id": user_ids, "original_message": str(err)},
                )
    ...
    def get_job(self, job_id):
        ...
        if job["owner"]["username"] != self.username:
            raise ServiceError(
                ...
                data={"job_id": job_id, "username": self.username},
            )
    ...
    def cancel_job(self, params):
        ...
        if job["state"]["status"] not in ["create", "queue", "run"]:
            return ServiceError(
                ...
                data={"job_id": job_id, "job_status": job["state"]["status"]},
            )
