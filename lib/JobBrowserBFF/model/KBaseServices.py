...
class KBaseServices(object):
    ...
    def get_user(self):
        ...
        if response.status_code != 200:
            ...
            try:
                ...
                return {"username": result["user"], "realname": result["display"]}
    ...
    def get_apps(self, apps_to_fetch):
        ...
        for app_id, tag, app_info, lookup_app_id in fetched_apps:
            ...
            app = {"id": lookup_app_id, "module_name": module, "function_name": name}
    ...
    def get_workspaces(self, workspace_ids):
        ...
        rpc = GenericClient(
            url=url, module="Workspace", token=self.token, timeout=self.timeout
        )
        ...
        for workspace_id in workspace_ids:
            try:
                [
                    ...
                    lockstat,
                    metadata,
                ] = rpc.call_func("get_workspace_info", {"id": workspace_id})
                ...
                workspace_infos.append({"id": workspace_id, "is_accessible": False})
    ...
    def get_client_groups(self):
        ...
        rpc = GenericClient(
            url=url, module="Catalog", token=self.token, timeout=self.timeout
        )
