...
class EE2Api(object):
    def __init__(self, url, token, timeout):
        ...
        self.rpc = GenericClient(
            url=self.url, module=self.module, timeout=self.timeout, token=self.token
        )
    ...
    def list_config(self):
        try:
            ...
            raise ServiceError(
                code=40000, message="Unknown error", data={"original_message": str(err)}
            )
    ...
    def check_jobs(self, params):
        try:
            ...
            raise ServiceError(
                code=1, message="Unknown error", data={"original_message": str(err)}
            )
