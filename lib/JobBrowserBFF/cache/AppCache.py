...
class AppCache:
    def __init__(
        self, path=None, narrative_method_store_url=None, upstream_timeout=None
    ):
        if path is None:
        ...
        if not isinstance(narrative_method_store_url, str):
            raise ValueError(
                'The "narrative_method_store_url" argument must be a string'
            )
    ...
    def load_for_tag(self, tag):
        ...
        result = rpc.call_func("list_methods", {"tag": tag})
