...
class JSONRPCHandler(BaseHTTPRequestHandler):
    ...
    def send_json_response(self, id, value):
        ...
        self.end_headers()
        data = {"id": id, "version": "1.1", "result": value}
    ...
    def send_error_response(self, id, value):
        ...
        self.send_header("Content-Type", "application/json")
        self.end_headers()
        data = {"id": id, "version": "1.1", "error": value}
        self._write_json(data)
    ...
    def do_POST(self):
        ...
        try:
            ...
            self.send_error(
                500, 'Cannot load service module "%s": %s' % (module, err.message)
            )
            return
...
class ServerController(object):
    ...
    def start(self):
        ...
        print(
            time.asctime(), "JSON RPC Server Starting at %s:%s" % (self.host, self.port)
        )
        try:
    ...
    def stop(self):
        self.httpd.server_close()
        print(
            time.asctime(), "JSON RPC Server Stopped at %s:%s" % (self.host, self.port)
        )
